...
Class FuncaoDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('funcao');
	}

	//cadastrar
	function cadastrar (funcao $obj) {
		$this->sql = sprintf("INSERT INTO funcao(descricao)
		VALUES('%s')",
	...
	//atualizar
	function atualizar (Funcao $obj) {
		$this->sql = sprintf("UPDATE funcao SET descricao = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
	...
	//buscarPorId
	function buscarPorId (Funcao $obj) {
		$this->sql = sprintf("SELECT * FROM funcao WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	function buscarFuncao ($busca) {
		$this->sql = "SELECT * FROM funcao where descricao like '%$busca%'";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Funcao' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM funcao limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Funcao' , 'ListarPaginado' ) );
		}else{
	...
	function deletar (Funcao $obj) {
		...
		$this->sql = sprintf("DELETE FROM funcao WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM funcao";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
