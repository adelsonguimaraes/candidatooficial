...
Class TipoliderDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('tipolider');
	}

	//cadastrar
	function cadastrar (tipolider $obj) {
		$this->sql = sprintf("INSERT INTO tipolider(descricao)
		VALUES('%s')",
	...
	//atualizar
	function atualizar (Tipolider $obj) {
		$this->sql = sprintf("UPDATE tipolider SET descricao = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
	...
	//buscarPorId
	function buscarPorId (Tipolider $obj) {
		$this->sql = sprintf("SELECT * FROM tipolider WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	function buscarTipo ($busca) {
		$this->sql = "SELECT * FROM tipolider WHERE descricao like '%$busca%'";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'TipoLider', 'buscarPorTipo' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	//listar
	function listar () {
		$this->sql = "SELECT * FROM tipolider";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Tipolider' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
				array_push($this->lista, $row);
			}
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM tipolider limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Tipolider' , 'ListarPaginado' ) );
		}else{
	...
	//deletar
	function deletar (Tipolider $obj) {
		$this->superdao->resetResponse();
		...
		$this->sql = sprintf("DELETE FROM tipolider WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM tipolider";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
