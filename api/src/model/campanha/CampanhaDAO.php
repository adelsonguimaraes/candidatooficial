...
Class CampanhaDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('campanha');
	}

	//cadastrar
	function cadastrar (campanha $obj) {
		$this->sql = sprintf("INSERT INTO campanha(nome, descricao)
		VALUES('%s', '%s')",
	...
	//atualizar
	function atualizar (Campanha $obj) {
		$this->sql = sprintf("UPDATE campanha SET nome = '%s', descricao = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getNome()),
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
	...
	//buscarPorId
	function buscarPorId (Campanha $obj) {
		$this->sql = sprintf("SELECT * FROM campanha WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	//listar
	function listar () {
		$this->sql = "SELECT * FROM campanha";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Campanha' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM campanha limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Campanha' , 'ListarPaginado' ) );
		}else{
	...
	function deletar (Campanha $obj) {
		...
		$this->sql = sprintf("DELETE FROM campanha WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM campanha";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
